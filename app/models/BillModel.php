...
class BillModel extends BasicModel {
    ...
    function getBills(BillObject $similar, $page, $itemsPerPage = null) : array {
        $list = array();
        if ($page <= 0) {
            $page = 1;
        }
        $limit = $itemsPerPage ?? 10; // Mặc định 10 bản ghi
        $offset = ($page - 1) * $limit;
    
        $sql = "SELECT * FROM tblbill 
                LEFT JOIN tblbillstatic ON tblbill.bill_static = tblbillstatic.billstatic_id ";
        $conditions = $this->createConditions($similar);
        if ($conditions == "") {
            return $list; // Trả về rỗng nếu không có điều kiện
        }
        $sql .= $conditions;
    ...
    function countBill(BillObject $similar) : int {
        ...
        $conditions = $this->createConditions($similar);
        if ($conditions == "") {
            return 0; // Trả về 0 nếu không có điều kiện
        }
        $sql .= $conditions;
        $sql .= ";";
    
        // Debug: In câu SQL để kiểm tra
        echo "<!-- Debug SQL (countBill): $sql -->";
    ...
    private function createConditions(BillObject $similar) {
        ...
        if ($user_id !== null && $user_id !== 0) {
            $out .= "bill_customer_id = " . intval($user_id) . " "; // Sử dụng intval để đảm bảo an toàn
        } else {
            // Nếu không có user_id, trả về điều kiện rỗng để tránh lấy toàn bộ dữ liệu
            return "";
        }
